...
// we temperalliy store Player 1's choice 
...
// function switches the game modes between CPU and pvp mode.
// This function will also reset values and updates UI.
...
function aSetMode(aNewMode) {
    ...
    if (aMode === "cpu") {
        ...
        //hide the player 2 controls
        aP2Section.style.display = "none";
        //update the instructions
        aModeHint.textContent =
            "Pick your move. The CPU will pick its move automatically.";
    } else {
...
function aGetWinner(aP1, aP2) {
    // check if its a tie if they picked the same choice.
...
function aHandleP1Pick(aChoice) {
    ...
    if (aMode === "cpu") {
        ...
        // show quick status while we are waiting for API call
